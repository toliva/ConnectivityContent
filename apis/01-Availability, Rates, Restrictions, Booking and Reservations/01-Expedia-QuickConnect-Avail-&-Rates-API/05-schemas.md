...
4 | @currency | String | O | 3-letter currency code assigned to property on Expedia Partner Central. Based on ISO 4217 specification.
Optional only when message is for rate change indicator updates, without any rate modification. | Minimum length  = 3, Must match the property currency defined on Expedia Partner Central, Must always be provided when update request includes rates.
