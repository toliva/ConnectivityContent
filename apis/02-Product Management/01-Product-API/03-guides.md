...
It is possible for a non-free extra bed to be returned with a surcharge but no amount. This case could happen when the sucharge has been created or updated by another mean; one that accepts a surcharge without amount. The Product-API will return the extra bed under this format. 
